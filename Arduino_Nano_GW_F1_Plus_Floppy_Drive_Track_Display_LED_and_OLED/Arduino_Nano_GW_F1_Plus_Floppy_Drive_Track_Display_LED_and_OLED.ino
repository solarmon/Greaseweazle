/***************************************************************************/
// GW F1 Plus (Gotek and 3.5 custom boards) Floppy Drive Tracker - LEDC68 and OLED
// by solarmon
// https://github.com/solarmon/Greaseweazle
//
// Supports following screen types:
// * LEDC68
...
// * Adafruit SSD1306
// * I2CScanner
// * Gotek LEDC68 - https://github.com/coopzone-dc/GotekLEDC68
/**************************************************************************/

const char *myDescription = "Greaseweazle F1 Plus (Gotek or 3.5) Track Display";
...
// Note: On Arduino Nano the D20/A6 and D21/A7 pins cannot be used as digital inputs, so cannot be used by digitalRead()
...
void setup()
{
  ...
  pinMode(PIN14_Pin, INPUT_PULLUP);
  pinMode(PIN12_Pin, INPUT_PULLUP);
    ...
    case LED:
      {
        ...
        while (1)
        {
          ...
          DIR_State = digitalRead(DIR_Pin); // LOW = STEP IN; HIGH = STEP OUT
          ...
          PIN14_State = digitalRead(PIN14_Pin); 
          PIN12_State = digitalRead(PIN12_Pin);
    ...
    case OLED:
      {
        ...
        while (1)
        {
          ...
          //Serial.print(F("### PIN14_State: "));
          //Serial.print(PIN14_State);
          //Serial.print(F(" | PIN12_State: "));
          //Serial.println(PIN12_State);
          
          // S0 (Assumed)

          if (PIN14_State == 1 && PIN12_State == 1)
          {
            switch(oled_h)
            {
              case 32:
            ...
            **/
            ...
            oled_display.print ("S0");
          }
          
          // S2 / A

          if (PIN14_State == 0 && PIN12_State == 1)
          {
            switch(oled_h)
            {
              case 32:
              {
                oled_display.setTextSize (1); 
                oled_display.setCursor (oled_w - (font_1_w * 5), 0); // position the cursor
                break;
              }
              case 64:
              {
                oled_display.setTextSize (2); 
                oled_display.setCursor (oled_w*0.75 - ((font_2_w*5)/2), oled_h-font_2_h); // position the cursor
                break;
              }
            }
            ...
            Serial.print(F("| PIN12_State: "));
            Serial.println(PIN12_State);
            Serial.println(F("S2/A"));
            Serial.println(F("..."));
            **/
            
            oled_display.print ("S2/A");
          }

           // S1 / B
           
          if (PIN14_State == 1 && PIN12_State == 0)
          {
            switch(oled_h)
            {
              case 32:
            ...
            Serial.print(F("| PIN12_State: "));
            Serial.println(PIN12_State);
            Serial.println(F("S1/B"));
            Serial.println(F("..."));
            **/
            
            oled_display.print ("S1/B");
          }
